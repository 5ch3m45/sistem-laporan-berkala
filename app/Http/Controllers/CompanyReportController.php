...
class CompanyReportController extends Controller
{
    ...
    {
        return view('company.report.index', compact('company'));
    }

    public function show(Company $company, Report $report)
    {
        return view('company.report.show', compact('company', 'report'));
    }

    public function create(Company $company)
    {
        return view('company.report.create', compact('company'));
    }

    public function create_company_report($report_id, $account_code, $value = 0) {
        return CompanyReport::create(compact('report_id', 'account_code', 'value'));
    }
    ...
    public function delete(Company $company, Report $report)
    {
        $company_reports = CompanyReport::where('report_id', $report->id)->get(); // get semua company_report berdasarkan id report
        foreach ($company_reports as $company_report) { // untuk semua compani report
            $company_report->delete(); // hapus
        }

        $report->delete(); // hapus report
        
        return back()->with(['success' => 'Data laporan berhasil dihapus.']); // kembali dan beri pesan sukses
    }
}
